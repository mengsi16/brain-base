...
### T57 — ingest-file 路径 `source_priority` 跨路径对齐补全

**问题**：T50 source_priority 跨档位优先级（P0-P3）字段引入时，`qa_persist.write_raw_one`（ask 路径）已注入 raw frontmatter，但 `nodes/ingest_file.py:frontmatter_node`（ingest-file 路径）字面量模板漏写该字段。后果是 user-upload 文档进 Milvus 时该字段缺失，ask 检索后做证据冲突仲裁（`qa_prompts.py:267` 文案 P0 > P1 > P2 > P3）时被默认当 P3（最低优先级），与设计意图（user-upload = P1，与 official-old 同级、高于 community）相反。

**修复**：`frontmatter_node` 字面量加 1 行 `source_priority: P1`（user-upload 不依赖 fetched_at 时效，按 `_compute_source_priority:204-205` 真值表固定 P1）。

**影响面**：

| 模块 | 改动量 |
|------|------|
| `brain_base/nodes/ingest_file.py:frontmatter_node` | +1 行字面量 |
| `tests/unit/test_ingest_file_frontmatter.py` | +1 测试文件（2 条守门断言） |
| `chunker` / `persistence` / `qa_persist` / `qa_prompts` / Milvus schema | **0 行**（下游通过 `chunker if val` 守卫与 inherit_fields 清单**自动获益**） |

**历史无字段文档批量补救**（运维步骤）：

```bash
# dry-run 看会改哪些 chunk
python bin/source-priority.py add-priority

# 确认无误后 --apply 落盘
python bin/source-priority.py add-priority --apply
```

**契约文档**：`md/research/2026-05-21-t57-ingest-file-link-cleanup.md`（5 契约面盘查表 + 修改前后字段对比 + R1-R6 风险审查 + 7 步执行流程 + 5 项验收标准 + 回滚计划）。

---

## 下一阶段路线

- **T58** E2E 基线测试（pending，最低优先级）：选 3-5 条典型问题落盘 e2e_trace.jsonl，未来回归任何破坏性改动一跑就知道。
