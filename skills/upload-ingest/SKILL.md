...
2. 用户把 PDF / DOCX / PPTX / XLSX / LaTeX / TXT / MD / 图片 文件交给助手并要求入库。
3. 用户给出目录路径，希望批量入库其中所有支持格式的文档。

在以下场景不要触发：

1. 用户只是让你阅读文档，并不要求入库。
2. 用户想抓取网页——那是 `get-info-agent` 的职责，不是本路径。
3. 用户文件格式不在支持列表（非 PDF/DOCX/PPTX/XLSX/LaTeX/TXT/MD/PNG/JPG/JPEG）——应当明确告知用户不支持并停下。
...
1. 文件存在。
2. 扩展名在支持列表：`.pdf / .docx / .pptx / .xlsx / .png / .jpg / .jpeg / .tex / .txt / .md / .markdown`。
3. 非空（`stat().st_size > 0`）。
...
1. `python bin/doc-converter.py check-runtime` — 确认 MinerU 和/或 pandoc 按需可用（PDF/DOCX/PPTX/XLSX/图片 依赖 MinerU；`.tex` 依赖 pandoc；TXT/MD 无外部依赖）。
...
7. 这一步**不做**分块、不生成合成 QA、不做任何预分块——那些全部是 `knowledge-persistence` 的职责，且必须由 Agent/LLM 执行。
